...
exports.restoreDefaultsPlugin = function restoreDefaultsPlugin(Model) {
    ...
    Model.DAO.prototype.restoreDefaults = function restoreDefaults() {
        // nothing to restore
    ...
    Model.hook('beforeUpdate', function (model) {
        ...
        fieldsToTrack.forEach(function (field) {
            changes.original[field] = changes.original[field] || model.previous(field);
            changes.current[field] = model[field];
        });

        model.setDataValue('_changes', changes);
    });
