...
exports.plugin = {
    bulkUpsertStream ($stream, options) {
        ...
        // ensure all attributes are defined and properly formatted as postgres expects
        const ensureRecordValues = (rec, attrs) => {
            _.forEach(attrs, attr => {
                let val = rec[attr.field];
                if (_.isUndefined(val)) {
                    // undefined -> null
                    val = null;
                } else if (_.isPlainObject(val) || ['json', 'jsonb'].indexOf(attr.type.toSql().toLowerCase()) >= 0) {
        ...
        // build model instances so hooks & validations happen
            ...
            .map(rec => ensureRecordValues(rec, attrsForInsert))
