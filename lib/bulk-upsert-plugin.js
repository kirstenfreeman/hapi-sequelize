...
exports.plugin = {
    bulkUpsertStream ($stream, options) {
        ...
        const insertOnConflict = conn => P.resolve(proceed).then(() => query(conn, `INSERT INTO "${this.tableName}"(${insertColumns}) SELECT ${insertColumns} FROM ${tempTableName} ON CONFLICT (${onConflictColumns}) DO UPDATE SET (${lhsUpdateColumns}) = ROW(${rhsUpdateColumns})`));
    ...
    bulkUpsert (records, options) {
        ...
        const updateQuery = util.format('WITH upd AS (UPDATE "%s" t SET (%s) = ROW(%s) FROM %s s WHERE %s RETURNING s."%s")',
            this.tableName,
            lhs,
            rhs,
            tempTableName,
