...
internals.createHandler = function(sequelize, defaults, route, routeOptions) {
    ...
    joi.validate(routeOptions, internals.optionsSchema(sequelize, defaults), function (err, validated) {
        ...
        if (validated.hasOwnProperty('scope')) {
            Model = Model.scope(validated.scope);
        }
    ...
    return function(req, reply) {
        // compile the options
        Sequelize.Promise.resolve(internals.queryOptions(req, Model, routeOptions))
            ...
            .then(function(options) {
                return Model.findAndCountAll(options, routeOptions.queryOptions);
            })
