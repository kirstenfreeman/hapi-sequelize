 ...
 * @return {string}
 */
internals.generateSlug = function(name, suffix) {
    return name ? slugify(name + (suffix ? '-' + suffix : '')).toLowerCase() : moniker.choose();
};
...
internals.slugPlugin = function(nameAttr, slugAttr, model) {
    ...
    model.generateSlug = function(name) {
        function nextSlug(pass) {
            var slug = internals.generateSlug(name, pass > 1 ? pass : '');

            return model.count({ where: { slug: slug } })
                .then(function (count) {
                    return count ? nextSlug(pass + 1) : slug;
                });
        }
