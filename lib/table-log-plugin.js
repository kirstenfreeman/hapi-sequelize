...
internals.createLogFn = function (sequelize, logTable, modelTable) {
    return sequelize.query(`CREATE OR REPLACE FUNCTION ${modelTable}_update_log() RETURNS TRIGGER AS $$ BEGIN INSERT INTO ${logTable} (id, "updatedAt") VALUES ('${modelTable}', NOW()) ON CONFLICT (id) DO UPDATE SET "updatedAt" = NOW(); RETURN NEW; END; $$ language plpgsql;`)
};
...
internals.createTrigger = function (sequelize, modelTable) {
    return sequelize.query(`CREATE TRIGGER log_update AFTER INSERT OR UPDATE OR DELETE ON ${modelTable} FOR EACH ROW EXECUTE PROCEDURE ${modelTable}_update_log()`);
};
...
module.exports = function (opts = {}) {
    ...
    return function (model) {
        ...
        if (!sequelize.models[opts.modelName]) internals.createModel(sequelize, opts);

        model.afterSync(function () {
            return internals.createLogFn(sequelize, opts.tableName, tableName)
